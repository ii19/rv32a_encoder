...
struct uint5_t {
    unsigned rd: S;
    operator auto() { return rd; }
    auto operator=(auto a) {
        if( a & 0xFFFFFFE0)
            throw std::runtime_error("register index must be in range 0-31");
        return rd=a;
    }
}; [[gnu::packed]];


//macro from x3/numeric/int.hpp
#define BOOST_SPIRIT_X3_INT_PARSER(int_type, name)                              \
typedef boost::spirit::x3::int_parser<int_type> name##type;                     \
constexpr name##type name = {};

BOOST_SPIRIT_X3_INT_PARSER(uint5_t<5>, uint5);

#undef BOOST_SPIRIT_X3_INT_PARSER

template<typename T>
auto as = [] ( auto s ) {
    return x3::rule<class tag, T> {s};
};
