...
static auto rule_reg = as<unsigned>("rule_reg") = 'x' >> int_;
...
// addi x0, x2, 1
static auto parser_i = as<instr_i>("second_parser_i") = mnemonic >> x3::as_parser(rule_reg) >> ','
                                                        >> x3::as_parser(rule_reg) >> ',' >> int_;
...
// sb x2, 2(x5)
static auto parser_s = as<instr_s>("parser_s") = s_mnemonic >> x3::as_parser(rule_reg) >> ','
                                                 >> int_ >> '(' >> x3::as_parser(rule_reg) >> ')';
...
static auto rule_label = x3::rule<class rule_label>{} = (*(~char_(':')) >> ":");
...
constexpr auto f
    = array<variant<decltype(ADD), decltype(ADDI), decltype(SLLI)>, 25>{ADD,  AND, XOR,  SLL,  SRL,
                                                                        OR,   SLT, SLTU, SLLI, SRLI,
                                                                        SRAI, SRA, ADDI, ANDI, ORI,
                                                                        XORI, LB,  LH,   LW,   LBU,
                                                                        LHU,  SB,  SH,   SW, JALR};
}

template<typename X1, typename X2, size_t... N>
constexpr void fold(std::index_sequence<N...> I, auto &&f, X1 &&t, X2 &&t1)
{
...
constexpr auto visitor = overload{
    []<typename T, typename U> requires(
        (same<T, decltype(SRAI)> &&same<U, instr_i>)
